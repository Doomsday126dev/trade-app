...
function fixture(env = {}) {
  let options, handler, calls = 0, verified = 0, rejectToken = false;
  ...
  const modules = {
    ...
    './reset': { createResetService: () => ({ run: async context => { calls++; assert.equal(context.appVerified, true); return { status: 'completed' }; } }) },
    './adapter': { createAdapter: () => ({}) }, './journal': { createJournal: () => ({}), createGcsStore: () => ({}) },
    './password': { createPasswordUpdater: () => () => {} }
  };
  ...
  return { options, handler, request, calls: () => calls, verified: () => verified, revoke: () => { rejectToken = true; } };
}
