(function(global){
  ...
  function renderTabs(snapshot){
    ...
    const counts=Object.fromEntries(LIST_TYPES.map(type=>[type,global.PogoDomain.publicSharePublication.intentEntries(snapshot,'lf',type).length]));
  ...
  function renderList(snapshot){
    ...
    const sections=global.PogoDomain.priorityValues.wantSections(list).map(section=>{
      const {entries,key,priority}=section,label=global.PogoDomain.priorityValues.wantSectionLabel(section,t,{recipient:true});
