...
function strLevelsHtml(strs,options={}){
  ...
  const t=localized?options.t:(key,params={})=>({
    'share.copy':'Copy','share.viewSearch':'View search','share.hideSearch':'Hide search','share.combinedOptions':'Combined options',
    'share.combinedAvailable':'{count} available','share.allPriorities':'All priorities',
    'share.priorityCombination':'{first} + {second}','strings.copyScoped':'Copy {label}',
    'strings.prioritySearch':'{priority} Priority Search String','strings.luckyDexSearch':'Lucky Dex Search String',
    'strings.shinyDexSearch':'Shiny Dex Search String','strings.xxlDexSearch':'XXL Dex Search String','strings.xxsDexSearch':'XXS Dex Search String'
  ...
  const copyButton=(value,label)=>`<button class="cpbtn" type="button" data-copy="${escAttr(value)}"${copyScope} onclick="copyStr(this.dataset.copy,this)" aria-label="${escAttr(t('strings.copyScoped',{label}))}" style="margin-left:auto"><svg class="ui-icon ui-icon-sm" aria-hidden="true"><use href="#ui-icon-copy"></use></svg><span>${escHtml(t('share.copy'))}</span></button>`;
  ...
  const prioritySearchLabel=p=>t('strings.prioritySearch',{priority:priorityLabel(p)});
  const dexSearchLabel=key=>t({LUCKY:'strings.luckyDexSearch',SHINY:'strings.shinyDexSearch',XXL:'strings.xxlDexSearch',XXS:'strings.xxsDexSearch'}[key]);
  ...
  return`<div class="str-levels">${['H','M','L'].filter(p=>strs[p]).map(p=>`
    <div class="str-level">
      <div class="str-level-hdr">
        <span class="badge ${p}"><span class="prio-mark">${p}</span>${escHtml(prioritySearchLabel(p))}</span>${strLenHtml(strs[p],panelOptions)}
        ${copyButton(strs[p],prioritySearchLabel(p))}
      </div>
      ${searchDisclosure(strs[p],prioritySearchLabel(p))}
      ${strWarnHtml(strs[p],panelOptions)}
    </div>`).join('')}${strs.LUCKY?`<div class="str-level lucky-str">
      <div class="str-level-hdr">
        <span class="combo-badge">${escHtml(dexSearchLabel('LUCKY'))}</span>${strLenHtml(strs.LUCKY,panelOptions)}
        ${copyButton(strs.LUCKY,dexSearchLabel('LUCKY'))}
      </div>
      ${searchDisclosure(strs.LUCKY,dexSearchLabel('LUCKY'))}
      ${strWarnHtml(strs.LUCKY,panelOptions)}
    </div>`:''}${strs.SHINY?`<div class="str-level shiny-str">
      <div class="str-level-hdr">
        <span class="combo-badge">${escHtml(dexSearchLabel('SHINY'))}</span>${strLenHtml(strs.SHINY,panelOptions)}
        ${copyButton(strs.SHINY,dexSearchLabel('SHINY'))}
      </div>
      ${searchDisclosure(strs.SHINY,dexSearchLabel('SHINY'))}
      ${strWarnHtml(strs.SHINY,panelOptions)}
    </div>`:''}${strs.XXL?`<div class="str-level xxl-str">
      <div class="str-level-hdr">
        <span class="combo-badge">${escHtml(dexSearchLabel('XXL'))}</span>${strLenHtml(strs.XXL,panelOptions)}
        ${copyButton(strs.XXL,dexSearchLabel('XXL'))}
      </div>
      ${searchDisclosure(strs.XXL,dexSearchLabel('XXL'))}
      ${strWarnHtml(strs.XXL,panelOptions)}
    </div>`:''}${strs.XXS?`<div class="str-level xxs-str">
      <div class="str-level-hdr">
        <span class="combo-badge">${escHtml(dexSearchLabel('XXS'))}</span>${strLenHtml(strs.XXS,panelOptions)}
        ${copyButton(strs.XXS,dexSearchLabel('XXS'))}
      </div>
      ${searchDisclosure(strs.XXS,dexSearchLabel('XXS'))}
