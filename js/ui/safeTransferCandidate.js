(function(global){
  ...
  function mountSafeTransferCandidate(container,{controller,t=(key,vars={})=>{
    ...
    container.innerHTML='<p data-safe-transfer-scope></p><p data-safe-transfer-status role="status" aria-live="polite"></p><div data-safe-transfer-commands></div>';
    ...
    function render(state){
      ...
      state.plan.commands.forEach((command,index)=>{
        ...
        field.readOnly=true;field.value=command.value;field.dataset.safeTransferCommand=String(index);
        button.type='button';button.textContent=t('copy',{part:index+1,total:state.plan.commands.length});button.dataset.safeTransferCopy=String(index);button.disabled=state.phase==='copying';
