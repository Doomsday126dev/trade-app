(function(global){
  ...
  function contextualSearchHtml(plan,{t,title}={}){
    ...
    const manual=plan.manual.map(entry=>{
      const qualifiers=[entry.mod,entry.variant,entry.backgroundLabel||entry.backgroundId,entry.gender,entry.maxType,entry.category==='wishlist'?'':entry.category||entry.type];
    ...
    return`<details class="contextual-search"><summary><svg class="ui-icon ui-icon-sm" aria-hidden="true"><use href="#ui-icon-search"></use></svg><span>${escHtml(title)} <small>${escHtml(label)}</small></span></summary><div class="contextual-search-body"><p>${escHtml(t('contextSearch.warning'))}</p>${!plan.total?`<p>${escHtml(t('contextSearch.empty'))}</p>`:''}${plan.unresolved?`<p role="status">${escHtml(t('contextSearch.unresolvedCount',{count:plan.unresolved}))}</p>`:''}${plan.parts.length>1?`<p>${escHtml(t('contextSearch.split',{count:plan.parts.length}))}</p>`:''}${plan.parts.map((value,index)=>`<div class="contextual-search-part"><button type="button" class="btn btn-secondary" data-contextual-copy="${escAttr(value)}" aria-label="${escAttr(t('contextSearch.copyPart',{part:index+1,total:plan.parts.length}))}"><svg class="ui-icon ui-icon-sm" aria-hidden="true"><use href="#ui-icon-copy"></use></svg>${escHtml(t('contextSearch.copyPart',{part:index+1,total:plan.parts.length}))}</button><textarea class="strbox" readonly rows="2" aria-label="${escAttr(label)}">${escHtml(value)}</textarea></div>`).join('')}<span class="contextual-copy-status" role="status" aria-live="polite"></span>${manual?`<details class="contextual-manual"><summary>${escHtml(t('contextSearch.manual',{count:plan.total}))}</summary><ul>${manual}</ul></details>`:''}</div></details>`;
  }
