(function(global){
  ...
  if(global.document)global.document.addEventListener('click',async event=>{
    ...
    catch{if(status.isConnected){panel.querySelector('.contextual-details').open=true;status.textContent=t('strings.copyFailed');const field=panel.querySelectorAll('textarea')[Number(button.dataset.copyIndex)];field?.focus();field?.select();}}
  });
