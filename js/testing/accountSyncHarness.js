(function(global){
  ...
  function createMemoryJournal(ownerUid,state=createMemoryJournalState(),clock=()=>Date.now()){
    ...
    async function enqueueOperations(operations,optimisticEntities=[]){
      ...
      operations.forEach((operation,index)=>{
        ...
        const record={ownerUid:owner,operationId:operation.operationId,operation,status:'pending',attempts:0,nextAttemptAt:0,createdAt:clock(),updatedAt:clock()};
    ...
    return Object.freeze({
      ...
      async retryBlocked(id){const record=state.operations.get(id);if(!record||record.status!=='blocked')return false;record.status='pending';record.attempts=0;record.nextAttemptAt=0;return true;},
  ...
  function createMultiDeviceHarness({ownerUid='uid-owner',crypto=global.crypto}={}){
    ...
    function createDevice(name,{state=createMemoryJournalState(),online=true}={}){
      ...
      Object.assign(runtime,{name,journal,controller,async start(){if(runtime.authenticated)await controller.activate();return runtime;},async disconnect(){runtime.online=false;await controller.deactivate();},async reconnect(){runtime.online=true;if(runtime.authenticated){await controller.activate();await controller.drain();}},async detachAuth(){runtime.authenticated=false;await controller.deactivate();},async reattachAuth(){runtime.authenticated=true;await controller.activate();if(runtime.online)await controller.drain();},async setOnline(value){runtime.online=value;if(value&&runtime.authenticated)await controller.drain();}});
