(function(global){
  ...
  function createMemoryJournal(ownerUid,state=createMemoryJournalState(),clock=()=>Date.now()){
    ...
    return Object.freeze({
      ...
      async setMeta(key,value){state.meta.set(key,value);},async listMeta(prefix=''){return [...state.meta.entries()].filter(([name])=>name.startsWith(prefix)).map(([name,value])=>({name,value}));},async getMeta(key){return state.meta.get(key)??null;},async removeMeta(key){state.meta.delete(key);},
