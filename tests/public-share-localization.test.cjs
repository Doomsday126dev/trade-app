...
test('release 2026-08-05.50 is coherent and contains no active .49 assets',()=>{
  const worker=source('sw.js'),release=source('js/domain/clientRelease.js');
  assert.match(html,/window\.__POGO_RELEASE_ID='2026-08-05\.50'/);
  assert.match(worker,/const RELEASE='2026-08-05\.50'/);
  assert.match(release,/RELEASE_ID='2026-08-05\.50'/);
  ...
  for(const src of firstParty)assert.equal(new URL(src,'https://example.test').searchParams.get('v'),'2026-08-05.50');
  assert.doesNotMatch(`${html}\n${worker}\n${release}`,/2026-08-05\.49/);
});
