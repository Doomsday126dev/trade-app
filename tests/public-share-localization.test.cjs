...
test('release 2026-08-05.20 is coherent and contains no active .19 assets',()=>{
  const worker=source('sw.js'),release=source('js/domain/clientRelease.js');
  assert.match(html,/window\.__POGO_RELEASE_ID='2026-08-05\.20'/);
  assert.match(worker,/const RELEASE='2026-08-05\.20'/);
  assert.match(release,/RELEASE_ID='2026-08-05\.20'/);
  const firstParty=[...html.matchAll(/<script\s+src="([^"]+)"/g)].map(match=>match[1]).filter(src=>!/^https?:/.test(src));
  assert.equal(firstParty.length,54);
  for(const src of firstParty)assert.equal(new URL(src,'https://example.test').searchParams.get('v'),'2026-08-05.20');
  assert.doesNotMatch(`${html}\n${worker}\n${release}`,/2026-08-05\.19/);
});
