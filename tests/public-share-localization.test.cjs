...
test('release 2026-08-22.54 is coherent and contains no active .53 assets',()=>{
  const worker=source('sw.js'),release=source('js/domain/clientRelease.js');
  assert.match(html,/window\.__POGO_RELEASE_ID='2026-08-22\.54'/);
  assert.match(worker,/const RELEASE='2026-08-22\.54'/);
  assert.match(release,/RELEASE_ID='2026-08-22\.54'/);
  ...
  for(const src of firstParty)assert.equal(new URL(src,'https://example.test').searchParams.get('v'),'2026-08-22.54');
  assert.doesNotMatch(`${html}\n${worker}\n${release}`,/2026-08-22\.53/);
});
