...
test('release 2026-08-27.73 is coherent and contains no active .66 assets',()=>{
  const worker=source('sw.js'),release=source('js/domain/clientRelease.js');
  assert.match(html,/window\.__POGO_RELEASE_ID='2026-08-27\.73'/);
  assert.match(worker,/const RELEASE='2026-08-27\.73'/);
  assert.match(release,/RELEASE_ID='2026-08-27\.73'/);
  ...
  for(const src of firstParty)assert.equal(new URL(src,'https://example.test').searchParams.get('v'),'2026-08-27.73');
