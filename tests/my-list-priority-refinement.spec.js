...
test('clean section, selected and Combine searches scopes hide details; clipboard failure still exposes exact copied bytes',async({page})=>{
  ...
  const before=await page.evaluate(()=>JSON.stringify(allData));
  const protectedPrefix='!4*&!traded&!shiny&CP-2500&!shadow&!purified&!background&';
  ...
  await expect(high.locator('[data-contextual-copy]')).toHaveAttribute('data-contextual-copy',protectedPrefix+'25');
  await page.locator('#wants-select-toggle').click();
  await high.locator('input').check();
  await expect(page.locator('#combined-search .contextual-details')).toBeHidden();
  await expect(page.locator('#combined-search [data-contextual-copy]')).toHaveAttribute('data-contextual-copy',protectedPrefix+'25');
  ...
  await expect(page.locator('.wants-custom-result [data-contextual-copy]')).toHaveAttribute('data-contextual-copy',protectedPrefix+'25,133');
  ...
  await expect(high.locator('textarea')).toHaveValue(protectedPrefix+'25');
  await expect(high.locator('textarea')).toBeFocused();
  expect(await high.locator('textarea').evaluate(n=>n.selectionEnd-n.selectionStart)).toBe((protectedPrefix+'25').length);
  await expect(high.locator('.contextual-copy-status')).toHaveClass(/is-error/);
  await expect(high.locator('.contextual-copy-status')).toHaveText(await page.evaluate(()=>i18nCore.t('strings.copyFailed')));
  expect(await page.evaluate(()=>JSON.stringify(allData))).toBe(before);
});
