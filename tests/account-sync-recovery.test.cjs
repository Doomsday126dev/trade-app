...
test('sanitized diagnostics bound unknown errors and contain no private values',()=>{
  const api=load(),privateValue='owner@example.com/trainer/Pikachu/op_secret',diagnostic=api.sanitizedDiagnostic(context(state({lastError:privateValue,lastErrorCategory:privateValue}),{recoveryOutcome:'failed',release:'2026-08-28.77'}));
  ...
  assert.equal(diagnostic.code,'account-sync/unknown');assert.equal(diagnostic.category,'projection');assert.equal(diagnostic.recoveryOutcome,'failed');assert.equal(diagnostic.release,'2026-08-28.77');assert.doesNotMatch(JSON.stringify(diagnostic),/owner@example|Pikachu|op_secret/);
});
