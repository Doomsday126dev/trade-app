...
async function installPublicReview(page,{theme='dark'}={}){
  ...
  await page.route('https://www.gstatic.com/firebasejs/*/firebase-app.js',fulfill('export function initializeApp(){return {}}'));
  await page.route('https://www.gstatic.com/firebasejs/*/firebase-app-check.js',fulfill('export class ReCaptchaEnterpriseProvider{};export function initializeAppCheck(){return {}};export async function getToken(){return {token:"synthetic-local-fixture"}}'));
  await page.route('https://www.gstatic.com/firebasejs/*/firebase-database.js',fulfill(`const projection=${JSON.stringify(projection)};export function getDatabase(){return {}};export function ref(_,path){return {path}};export async function get(target){globalThis.__reviewPublicReads=(globalThis.__reviewPublicReads||[]).concat(target.path);return {exists:()=>true,val:()=>projection}}`));
