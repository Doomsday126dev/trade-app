...
test('retired apply mode cannot bypass the coordinated owner endpoint',async()=>{const fake=adapter();const before=clone(fake.data);await assert.rejects(tool.executeReset({adapter:fake,options:options({apply:true}),env}),{code:'reset/cli-retired'});assert.deepEqual(fake.data,before);assert.equal(fake.writes.length,0)});
...
test('retired apply mode rejects before any adapter read or mutation',async()=>{await assert.rejects(tool.executeReset({adapter:new Proxy({}, {get(){throw new Error('Adapter must not be touched')}}),options:options({apply:true}),env}),{code:'reset/cli-retired'})});
...
test('CLI accepts one read-only username and rejects every apply request',()=>{assert.throws(()=>tool.parseArgs(['--username',USERNAME,'--username','Other']),{code:'reset/unsupported-option'});assert.throws(()=>tool.validateOptions(options({username:`${USERNAME},Other`}),env),{code:'reset/invalid-username'});assert.equal(tool.parseArgs(['--username',USERNAME]).apply,false);assert.doesNotThrow(()=>tool.validateOptions(options(),{}));assert.throws(()=>tool.validateOptions(options({apply:true}),{}),{code:'reset/cli-retired'})});
...
test('retired production adapter has no identity or product mutation capability',()=>{assert.doesNotMatch(tool.createAdminAdapter.toString(),/\.createUser\(|\.updateUser\(|\.deleteUser\(|updateAuthPassword|updateAppPin|\.update\(/)});
