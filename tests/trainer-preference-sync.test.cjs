...
function validLocal(){return{favorites:[{displayName:'Trainer A',key:'trainer a',tagIds:['tag_a'],note:'Private',createdAt:10,updatedAt:20}],tags:{tag_a:{label:'Group A',createdAt:10,updatedAt:20}},recent:[{displayName:'Trainer A',key:'trainer a',openedAt:30}],snapshots:{'trainer a':{seenAt:30,snapshot:{lists:{wishlist:{one:{p:'H'}},dynamax:{},gmax:{},costumes:{}}}}}};}

test('sync contract keeps locale and transient state device-only',()=>{
  const sync=load().PogoDomain.trainerPreferenceSync;
  assert.ok(sync.SYNCABLE_PRIVATE_DATA.includes('private-trainer-notes'));
...
test('executable conflict matrix matches independent metadata and tombstone behavior',()=>{
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(sync.CONFLICT_MATRIX)),{
    favoriteAddFavoriteAdd:'merge-earliest-timestamp-preserved',favoriteDeleteMetadataEdit:'tombstone-wins',noteEditNoteEdit:'explicit-user-conflict',tagRenameTagRename:'explicit-user-conflict',tagDeleteAssignment:'tombstone-wins',offlineEditNewerRemoteEdit:'reject',staleSchemaClientCurrentServer:'reject',accountSwitchPendingOperation:'reject'
  });
  const favorite={trainerName:'Trainer A',addedAt:10,revision:1,updatedAt:20,operationId:'operation-current1',deleted:false};
  const metadata={note:'old',tagIds:[],revision:1,updatedAt:20,operationId:'operation-current2',deleted:false};
  ...
  assert.equal(sync.resolveMetadataMutation(metadata,operation('metadata-upsert',{operationId:'operation-metadata1',baseRevision:1,payload:{note:'new',tagIds:[]}})).value.note,'new');
});

test('concurrent notes require intervention and deleted tags cannot be assigned',()=>{
  const sync=load().PogoDomain.trainerPreferenceSync,current={note:'old',tagIds:[],revision:2,updatedAt:100,operationId:'operation-current2',deleted:false};
  const stale=sync.resolveMetadataMutation(current,operation('metadata-upsert',{operationId:'operation-0000004',baseRevision:1,payload:{note:'new',tagIds:[]}}));
  assert.equal(stale.error.code,'trainer-preference-sync/conflict');
  const invalid=sync.resolveMetadataMutation(null,operation('metadata-upsert',{payload:{note:'private',tagIds:['tag_old']}}),{tags:{tag_old:{active:false,deleted:true}}});
...
test('migration counts normalized actual records and validates per-history snapshot bounds',()=>{
  ...
  const plan=sync.buildMigrationPlan({activeIdentity:owner,partitionIdentity:owner,localSchemaVersion:2,local,server:{metadata:{revision:4}},serverHydrated:true,hydrationGeneration:2,activeGeneration:2});
  ...
  assert.equal(sync.buildMigrationPlan({activeIdentity:owner,partitionIdentity:owner,localSchemaVersion:2,local:oversized,serverHydrated:true,hydrationGeneration:1,activeGeneration:1}).error.code,'trainer-preference-sync/migration-too-large');
});
...
test('cross-account and newer-schema migrations are rejected',()=>{
  ...
  assert.equal(sync.buildMigrationPlan({activeIdentity:owner,partitionIdentity:{uid:'viewer-b',username:'ViewerB'},localSchemaVersion:2,serverHydrated:true,hydrationGeneration:1,activeGeneration:1}).error.code,'trainer-preference-sync/partition-mismatch');
  assert.equal(sync.buildMigrationPlan({activeIdentity:owner,partitionIdentity:{uid:'viewer-a',username:'ＶｉｅｗｅｒＡ'},localSchemaVersion:2,serverHydrated:true,hydrationGeneration:1,activeGeneration:1}).error.code,'trainer-preference-sync/partition-mismatch');
  assert.equal(sync.buildMigrationPlan({activeIdentity:owner,partitionIdentity:owner,localSchemaVersion:2,server:{metadata:{schemaVersion:2}},serverHydrated:true,hydrationGeneration:1,activeGeneration:1}).error.code,'trainer-preference-sync/server-schema-newer');
  assert.equal(sync.buildMigrationPlan({activeIdentity:owner,partitionIdentity:owner,localSchemaVersion:2,local:{recent:Array.from({length:31},(_,index)=>({displayName:`Trainer ${index}`,openedAt:index}))},serverHydrated:true,hydrationGeneration:1,activeGeneration:1}).error.code,'trainer-preference-sync/migration-too-large');
});
