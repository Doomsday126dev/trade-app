...
test('canonical builder preserves current trim, minimum, optional-note, timestamp and pending semantics',()=>{
...
test('username validation accepts Unicode and internal spaces but rejects missing short wrong-type and control values',()=>{
  for(const username of ['ポケモン','Ä Trainer','A B'])assert.equal(contract.build({rawUsername:username,now:1,randomValue:0.1}).ok,true,username);
...
test('note remains optional and unbounded pending historical evidence while controls and non-strings fail',()=>{
  ...
  assert.equal(contract.build({rawUsername:'U'.repeat(5000),rawNote:veryLong,now:1,randomValue:0.1}).ok,true);
});

test('unapproved 32 and 280 limits and timestamp skew are not silently enforced',()=>{
  const source=readFileSync(path.join(root,'js/domain/requestAccess.js'),'utf8');
  assert.doesNotMatch(source,/length\s*(?:<=|<)\s*(?:32|280)/);
