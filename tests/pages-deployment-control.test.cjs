...
test('reviewed frontend allowlist stays at 80 files and excludes control/private trees',()=>{
  const result=validator.validateReleaseCoherence(root,{expectedReleaseId:RELEASE_ID});
  assert.equal(result.files.length,80);assert.equal(result.scriptCount,72);
...
test('schema 2 artifact records distinct runtime and control provenance without changing runtime digest',()=>{
  ...
  assert.equal(builder.walk(output).length,81);fs.rmSync(output,{recursive:true,force:true});
});
...
test('postdeploy requires schema 2 and exact runtime/control provenance',async()=>{
  ...
  assert.equal(result.sourceSha,RUNTIME_SHA);assert.equal(result.deploymentSelector,SELECTOR);assert.equal(result.scriptCount,72);
