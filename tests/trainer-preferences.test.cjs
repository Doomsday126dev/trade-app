...
test('repository never exposes direct Favorite writes and gates remaining exact writes',async()=>{
  ...
  const writable=factory({enabled:true,writesEnabled:true,readExact:async()=>{},listenExact:()=>{},transactionExact:(p)=>paths.push(p)});writable.mutateTrainerMetadata('viewer-a','owner-a',()=>{});writable.mutateMetadata('viewer-a',()=>{});assert.equal(typeof writable.mutateFavorite,'undefined');assert.equal(typeof window.PogoData.trainerPreferencesRepository.favoritePayload,'undefined');assert.equal(paths.some(path=>path.includes('/favoriteTrainers/')),false);assert.ok(paths.includes('userPreferences/viewer-a/trainerMetadata/owner-a'));assert.ok(paths.includes('userPreferences/viewer-a/metadata'));assert.equal(typeof writable.finalizeMigration,'undefined');assert.equal(typeof writable.saveTag,'undefined');assert.equal(typeof writable.saveHistory,'undefined');
});
