...
`scripts/pages/frontend-files.json` is the reviewed source of truth. It contains exactly 72 runtime files: `index.html`, `manifest.json`, `sw.js`, all 64 HTML-loaded first-party scripts, and five referenced icons. It excludes workflows, deployment scripts, tests, docs, functions, private/local files, logs, reports, `node_modules`, and temporary output.
