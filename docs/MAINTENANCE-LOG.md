...
Fixed My List row key handling so Pokémon names with apostrophes, such as `Farfetch'd`, keep their real stored key in row data while inline button handlers use separately escaped JavaScript strings. Single-item removals now queue an item-level delete instead of rewriting the whole list, reducing stale overwrite risk when removing one Pokémon, and the add-dropdown cache refreshes so a removed Pokémon can be re-added immediately. No Firebase rules, auth flow, Pokémon data shape, inventory/offers/schedule writes, share-link behavior, or community scoping changed.

## 2026-08-03 - Codex - Trainer-first global app Phase 0 plan (docs-only)

Added `docs/GLOBAL-APP-PLAN.md` to capture a proposed global, trainer-first product architecture before any implementation. The initial release boundary is trainer-name search, one public trainer profile/list at a time, pairwise list matching inside that profile, and comparison-specific search strings. The current Pokémon-first Browse layout, Pokémon-to-trainer global discovery, and automatic all-user matching are explicitly deferred unless backed by bounded server-maintained indexes. The plan also records the proposed My List / Find Trainer / Strings / Events / Settings navigation, UID-versus-trainer-handle identity model, visibility/privacy choices, on-demand Firebase read shape, and preservation-first Inventory/Offers/Schedule retirement strategy. `docs/AUTH-PLAN.md` now warns that its older stable-username identity invariant must be reconciled before auth implementation. No app code, Firebase data, Firebase rules, authentication behavior, feature flags, production UI, or existing records changed.

## 2026-08-03 - Codex - Global Phase 0 policies and free-tier plan (docs-only)

Recorded the initial global-app policies in `docs/GLOBAL-APP-PLAN.md`: new profiles default private; Public, Unlisted, and Private visibility remain user-controlled; every contact field defaults private and has independent visibility; authentication email is never projected automatically; anonymous visitors may view published profiles/lists but pairwise comparison initially requires a signed-in My List; self-service normalized trainer-name claims are allowed with atomic uniqueness, existing-user priority, provider-link collision guards, rename cooldown/history, recovery, and impersonation protections; and legacy Inventory/Offers/Trades/Schedule/community data receives a 90-day rollback window, 12-month read-only access period, verified archive/export process, and no automatic conversion or deletion. Added a Firebase free-tier section separating confirmed quotas from rough workload estimates, identifying the Spark 100-connection and 10-GB/month download limits as likely constraints, and defining bounded search, payload, listener, caching, abuse, and monitoring safeguards. Updated `docs/AUTH-PLAN.md` to make clear that its older stable-username identity target is not the proposed global identity model. No application code, UI, tests, Firebase data/rules, authentication behavior, subscriptions, or production records changed.

## 2026-08-03 - Codex - Global identity and migration decisions (docs-only)

Recorded the read-only audit of 35 current trainer names (zero collisions under the candidate trim + NFKC + case-insensitive comparison), while leaving the final normalizer provisional pending fixture tests. Clarified verified-provider publishing, provider-first recovery and frozen impersonation review, quantity-private public For Trade projections, archived legacy communities, anonymous-comparison deferral, monitoring-based Spark/Blaze decisions, and the shortened legacy-backup retention window. Added a sequence of small implementation commits with path, test, and rollback boundaries; the current community authorization vulnerability is explicitly scheduled for the second commit, immediately after rules-emulator coverage and before global identity work. No application code, UI, tests, Firebase data/rules, authentication behavior, subscriptions, or production records changed.
...
## 2026-08-03 - Codex - Emulator-only global identity contract candidate

Added a separate, undeployed Firebase Emulator contract for UID-owned accounts, private trainer-handle reservations, private profile/contact settings, allowlisted public profile/list projections, opaque unlisted shares with a private owner index, and an admin-managed legacy username index. Every new data write is gated by missing/false-by-default `globalIdentityConfig/writesEnabled`; no production client references the paths. Tests explicitly characterize that the current authenticated root read still exposes proposed private nodes, so the fixture is marked unsafe to deploy or seed until global subscriptions are removed and narrow-read production rules are live and verified. Revised the roadmap to split the former migration step into a local-only dry run before cutover and a separately approved production seed after private-path isolation. Added cross-cutting modularity, maintainability, and internationalization acceptance criteria, including a locale-catalog/module-boundary milestone before substantial global profile UI. Corrected the documented trainer-name audit count from 44 to the verified 35. No `index.html`, deployed Firebase rules/configuration, production data, authentication, UI, or current application behavior changed.

## 2026-08-03 - Codex - Local-only UID/handle reconciliation candidate

Added a modular, read-only reconciliation dry run for `loginDirectory`, `users`, `authIndex`, protected `admins`, and a separate sanitized local Firebase Auth input. Conflict-first classification keeps ambiguous, incomplete, disabled, transformed, inactive, orphaned, and protected records out of automatic readiness; every detailed record remains `seedEligible: false`. Production collection is restricted to exactly four GET requests with explicit target confirmation, while private 0600 reports stay under the git-ignored `.local/uid-handle-audits/` directory and console output remains aggregate-only. The tool creates no migration manifest, Firebase update, reservation, rollback write plan, or seed command, and no production read or write was performed while preparing this candidate.

## 2026-08-03 - Codex - Local-only identity conflict review candidate

Added offline-only conflict diagnostics and a private review-artifact workflow for duplicate/multi-UID mappings, UID mismatches, protected admins, unassociated Auth identities, passive-login candidates, and legacy holds. The pure domain helper organizes allowlisted source evidence without choosing an authoritative UID; local input and reporting modules enforce source-hash staleness checks, approved roots, git-ignored paths, and `0600` output. Every decision starts `unreviewed`, every record remains `seedEligible:false`, and the artifact contains no Firebase write payload, repair command, seed manifest, reservation, or ownership decision. No production read, write, rule, authentication, UI, or active client behavior changed; client read/subscription refactoring remains the next major milestone.

## 2026-08-03 - Codex - Firebase read boundary Phase 1 candidate

Added an inert Firebase client/repository boundary, explicit listener ownership manager, static read-surface registry, immutable cache adapters, and a minimal locale-catalog foundation. `NARROW_READ_CLIENT_ENABLED` remains false, all existing broad subscriptions and UI data sources remain active and unchanged, and the authenticated root read is not altered. New checks lock the current Firebase read inventory and verify listener deduplication, cleanup, stale-callback suppression, exact repository paths, cache isolation, and locale fallback. Firebase debug logs are now ignored without deleting or inspecting them. No Firebase rules, production data, global identity records, or user-visible behavior changed.
...
## 2026-08-04 - Codex - Owned-data exact-mode controlled validation

Validated the disabled owned-data exact-read path in a same-browser production session using a temporary in-memory override only. TestUser and the protected owner both loaded their exact profile, wishlist, Dynamax, Gigantamax, Costumes/Others, Inventory, auth-index, membership, and pending-decrement surfaces with one listener per account session and zero managed-listener errors. Logout cleared the protected cache and pending queue while preserving the public login directory; switching to the owner exposed no TestUser records or queued writes. No list or inventory data was edited, the override was removed after testing, and production defaults remain `NARROW_READ_CLIENT_ENABLED=false` and `LEGACY_BROAD_READS_ENABLED=true` pending migration of the broad Browse, Strings, Inventory Community Browse, Schedule, Offers, and Admin consumers.

## 2026-08-04 - Codex - Trainer-first exact-read activation candidate

Activated the validated exact owned-data client and disabled ordinary-session broad startup reads. The interim navigation now keeps My List, trainer-name/share-link lookup, own Strings, Events, read-only Legacy Inventory with CSV export, Settings, and protected owner Admin. Pokémon-first Browse, community-wide Strings enumeration, Inventory Community Browse, Trade Match, Offers, personal Schedule, and member community switching are retired from the active UI; their Firebase records and legacy code remain untouched for rollback. Selected trainers load and update only through `publicShares/{username}`, with signed-in list comparison performed locally against the viewer's exact owned lists and no private-record fallback. Retained owner maintenance collections start only after the protected owner opens Admin, use the `legacyAdmin` listener scope, and stop when Admin closes, on logout, or on auth loss. Firebase rules, schemas, and production data are unchanged.

## 2026-08-05 - Codex - Trainer-first production validation (docs-only)

Completed the focused deployed validation at clean repository commit `d7cefda`; no rollback was required. A normally reloaded warm client coherently loaded all 39 first-party modules at release `2026-08-05.1`, with no mixed-version failure. Partial, mixed-case, exact, fast-Enter, keyboard, mouse, and touch trainer search passed with canonical capitalization. The existing `ScoopskiPotat0` account completed exact owned-data hydration and explicitly republished through **Export -> Copy shareable link**; authenticated Find Trainer and the anonymous read-only share both rendered all 71 entries (High 36, Medium 19, Low 16). TestUser's realtime reversible priority edit updated the public share and was restored to its exact original list and category counts. Favorites, recents, My List strings, Events, Legacy Inventory, same-browser account isolation, protected Admin listener open/close/reopen lifecycle, responsive checks at 375, 430, 768, 1024, and 1440 px, and final anonymous directory/share access all passed. There were zero browser errors, Firebase permission errors, listener errors, or cache-reset loops.

Approved production writes were limited to `publicShares/ScoopskiPotat0` for explicit publication and, for TestUser's temporary priority edit plus restoration, `wishlist/TestUser`, `users/TestUser/lastUpdated`, `users/TestUser/lastSeen`, and `publicShares/TestUser`. All reversible TestUser changes were restored. No Firebase rules, identities, memberships, private cross-trainer paths, schemas, or unrelated records changed. Two bounded limitations remain recorded: browser tooling did not directly inspect the raw `publishedListTypes` field, although the resulting 71-entry publication passed the deployed schema validator; and an additional favorite-target production mutation was not forced solely to exercise unread diffs, while baseline/seen behavior passed and mutation cases remain covered by automated tests.
...
## 2026-08-05 - Codex - Admin Security panel production validation

Validated release `2026-08-05.2` in production with a normal warm-client reload. All 39 first-party assets loaded coherently, with no mixed-version warning, cache loop, browser error, or Firebase error. The protected owner retained Admin access. The Security panel displayed only the narrow-read deployment status, the `2026-08-05 10:05:15 EDT` deployment timestamp, candidate SHA-256, reviewed repository path, rollback readiness, and the warning that production rules must be managed from reviewed repository artifacts. It exposed no raw rules JSON, textarea, owner UID, obsolete broad authenticated-read rule, or raw-rules copy action.

**Copy candidate SHA** returned exactly `e0632a98ed106117f03e61da0446ef4b2c2e6ed02ea8c6f1c498a0e7edcb17bf`, and **Copy reviewed path** returned exactly `tests/firebase/database.rules.narrow-read.json`, with no added labels, whitespace, JSON, identifiers, or other content. Admin close/reopen lifecycle validation returned the same 52 rows with one Security panel and no duplicate rows, controls, rendering, listeners, or warnings. After owner logout, authoritative anonymous directory suggestions loaded and the exact anonymous read-only public share rendered all 71 entries: 36 High, 19 Medium, and 16 Low.

This validation performed zero production mutations and changed no Firebase rules, schemas, data, identities, or feature flags. No rollback was required. Final verdict: release `2026-08-05.2` is healthy in production.
