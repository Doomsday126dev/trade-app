...
Rollout remains owner only with 24 hours observation, then two known-clean accounts with 48 hours observation, then one explicitly human-selected, privately supplied cohort of exactly five eligible accounts. D3 tooling validates those exact five and never discovers, ranks, substitutes, or selects from a broader production population. Advancement requires zero conflicts, zero legacy regression, exact source fingerprints, clean logs, and a proven rollback. No production candidate classification is performed by this work.
