...
Cross-device sync is an owner-only private-state layer for the existing account UID. It is additive, allowlisted for one production owner canary in `.68`, and does not change authentication, public-share authority, or retired inventory paths.
