...
`2026-08-05.43`. Browser storage is device-local. It is not cross-device sync and
...
| `pogoSyncQueue_v2` | Envelope schema 2, exact `{uid, username}` owner | Local write queue and flush | Cleared on logout or different-owner activation | Invalid/wrong schema resets; legacy `pogoSyncQueue_v1` is discarded, never adopted; bounded by current pending narrow writes | May contain pending account list changes |
