...
| `pogoSyncQueue_v2` | Envelope schema 2, exact `{uid, username}` owner | Local narrow-write queue and flush; each My List action is one atomic owner-root `update()` patch, while legacy whole-list replacements are retained in a non-flushing quarantine | Cleared on logout or different-owner activation | Invalid/wrong schema resets; malformed, path-mismatched, or foreign-owner entries are discarded; legacy `pogoSyncQueue_v1` is discarded, never adopted; exact whole-list paths are quarantined before flush; quarantine is limited to one entry for each of the four owner list roots | May contain pending account list changes and owner-bound quarantined recovery evidence; no credentials or tokens |
