...
| `pogoSyncQueue_v2` | Envelope schema 2, exact `{uid, username}` owner | Local item-level write queue and flush; legacy whole-list replacements are retained in a non-flushing quarantine | Cleared on logout or different-owner activation | Invalid/wrong schema resets; legacy `pogoSyncQueue_v1` is discarded, never adopted; exact whole-list paths are quarantined before flush; bounded by current pending narrow writes | May contain pending account list changes and owner-bound quarantined recovery evidence |
