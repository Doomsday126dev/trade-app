...
function advance(state, evidence) {
  ...
  if (position < 0 || position === STAGES.length - 1) throw new Error('transition/stage-invalid');
  ...
  const checks = {
    ...
    'permanent-writers-closed': () => evidence.permanentRulesReadbackDigest === policy.candidateRulesSha256 &&
